...
import type { ModelOption, Channel, FileAttachment, ChatMessage } from '@proma/shared'
...
export function ModelSelector({
    ...
    // 内部选中模型状态（合并 atom）
    const internalSelectedModel = React.useMemo(() => {
    ...
    const handleSelect = (option: ModelOption): void => {
        ...
        if (currentSessionId) {
            window.electronAPI
                .updateAgentSessionModel(currentSessionId, option.channelId, option.modelId)
                .then((updated) => {
                    setSessions((prev) =>
